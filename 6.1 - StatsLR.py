...
def stats(file_path_lemma, file_path_stem):
    ...
    pos_labels_lemme = list(pos_count_lemme.keys())
    pos_values_lemme = list(pos_count_lemme.values())
    pos_labels_stem = list(pos_count_stem.keys())
    pos_values_stem = list(pos_count_stem.values())
    
    all_pos_labels = list(set(pos_labels_lemme + pos_labels_stem))                          # Assurer que les labels sont les mêmes pour les deux graphiques
    
    # Ajouter 0 pour les POS tags manquants dans un des deux ensembles
    pos_values_lemme = [pos_count_lemme.get(label, 0) for label in all_pos_labels]
    pos_values_stem = [pos_count_stem.get(label, 0) for label in all_pos_labels]
    
    fig_pos = go.Figure(data=[
        go.Bar(x=all_pos_labels, y=pos_values_lemme, name='Lemmatisation', marker_color='skyblue'),
    ...
    fig.add_trace(go.Histogram(x=list(lemmes_count.values()), nbinsx=30, name='Lemmatisation', marker_color='skyblue'), row=1, col=1)
    fig.add_trace(go.Histogram(x=list(stems_count.values()), nbinsx=30, name='Racinisation', marker_color='lightcoral'), row=1, col=2)
